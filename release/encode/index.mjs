...
// dist/jls_es6_encoder.js
async function CharLSEncoder(moduleArg = {}) {
  var moduleRtn;
  ...
  function findWasmBinary() {
    if (Module["locateFile"]) {
      return locateFile("jls_es6_encoder.wasm");
    }
    return new URL("jls_es6_encoder.wasm", import.meta.url).href;
  }
...
var jls_es6_encoder_default = CharLSEncoder;
...
async function decode(imageData, width, height, components, bitsPerSample, interleaveMode = -1, nearLossless = 0) {
  if (!charls) {
    charls = await jls_es6_encoder_default({
      locateFile: (path) => {
        console.log({ path });
